...
function gateKeeper(req, res, next) {
  let xHeader, xUser;
  for (xHeader in req.headers) {
    if (xHeader == "x-username-and-password") {
      let userInfo = queryString.parse(req.headers[xHeader]);
      for (xUser in USERS) {
        if (userInfo.user == USERS[xUser].userName) {
          req.user = USERS[xUser];
          next();
        }
      }
    }
...
app.use(morgan('common')); // log all requests
